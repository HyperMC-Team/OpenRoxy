...
implements Packet<INetHandlerPlayClient> {
    ...
    private Class386.Extracted[] chunksData;
    private boolean isOverworld;

    public Class396() {
    }

    public Class396(List<Chunk> chunks) {
        int i = chunks.size();
        this.xPositions = new int[i];
        this.zPositions = new int[i];
        this.chunksData = new Class386.Extracted[i];
        this.isOverworld = !chunks.get((int)0).getWorld().provider.getHasNoSky();
        for (int j = 0; j < i; ++j) {
            Chunk chunk = chunks.get(j);
            Class386.Extracted s21packetchunkdata$extracted = Class386.getExtractedData(chunk, true, this.isOverworld, 65535);
    ...
    public void readPacketData(PacketBuffer buf) throws IOException {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            this.chunksData[j] = new Class386.Extracted();
            this.chunksData[j].dataSize = buf.readShort() & 0xFFFF;
            this.chunksData[j].data = new byte[Class386.func_180737_a(Integer.bitCount(this.chunksData[j].dataSize), this.isOverworld, true)];
        }
        for (int k = 0; k < i; ++k) {
