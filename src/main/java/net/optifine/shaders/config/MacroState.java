    ...
    private boolean eval(String str) {
        ...
        block2: do {
            ...
            while (matcher2.find()) {
                ...
                int k = matcher2.end();
                str = ((String)str).substring(0, j) + " " + s1 + " " + ((String)str).substring(k);
                flag = true;
                ++i;
                continue block2;
            }
        ...
        if (i >= 100) {
            Config.warn("Too many iterations: " + i + ", when resolving: " + (String)str);
            return true;
        }
        try {
            ...
            IExpression iexpression = expressionparser.parse((String)str);
            ...
            throw new ParseException("Not a boolean or float expression: " + String.valueOf((Object)iexpression.getExpressionType()));
        }
        catch (ParseException parseexception) {
            Config.warn("Invalid macro expression: " + (String)str);
