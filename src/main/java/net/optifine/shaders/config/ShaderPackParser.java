...
public class ShaderPackParser {
    ...
    public static BufferedReader resolveIncludes(BufferedReader reader, String filePath, IShaderPack shaderPack, int fileIndex, List<String> listFiles, int includeLevel) throws IOException {
        ...
        while (true) {
            ...
            if ((matcher1 = PATTERN_INCLUDE.matcher((CharSequence)s1)).matches()) {
                ...
                if (((String)s1).endsWith("\n")) {
                    s1 = ((String)s1).substring(0, ((String)s1).length() - 1);
                }
                Object s5 = "#line 1 " + l + "\n";
                if (((String)s1).startsWith("#version ")) {
                    s5 = "";
                }
                s1 = (String)s5 + (String)s1 + "\n#line " + (k + 1) + " " + fileIndex;
            }
            if (j >= 0 && ((String)s1).contains(ShaderMacros.getPrefixMacro())) {
                ShaderMacro[] ashadermacro = ShaderPackParser.findMacros((String)s1, ShaderMacros.getExtensions());
                for (int i1 = 0; i1 < ashadermacro.length; ++i1) {
                    ShaderMacro shadermacro1 = ashadermacro[i1];
                    set.add(shadermacro1);
                }
            }
            chararraywriter.write((String)s1);
    ...
    private static CustomUniform parseCustomUniform(String kind, String name, String type, String src, Map<String, IExpression> mapExpressions) {
        try {
            ...
            if (uniformtype == null) {
                SMCLog.warning("Unknown " + kind + " type: " + String.valueOf((Object)uniformtype));
                return null;
            }
            ...
            if (!uniformtype.matchesExpressionType(expressiontype)) {
                SMCLog.warning("Expression type does not match " + kind + " type, expression: " + String.valueOf((Object)expressiontype) + ", " + kind + ": " + String.valueOf((Object)uniformtype) + " " + name);
                return null;
            }
