...
public enum ShaderParameterFloat {
    ...
    private ShaderParameterFloat(ShaderUniformBase uniform) {
        ...
        if (!ShaderParameterFloat.instanceOf(uniform, ShaderUniform1f.class, ShaderUniform1i.class)) {
            throw new IllegalArgumentException("Invalid uniform type for enum: " + String.valueOf((Object)this) + ", uniform: " + uniform.getClass().getName());
        }
    }
    ...
    public float eval(int index1, int index2) {
        if (this.indexNames1 == null || index1 >= 0 && index1 <= this.indexNames1.length) {
            if (this.indexNames2 == null || index2 >= 0 && index2 <= this.indexNames2.length) {
                ...
                throw new IllegalArgumentException("Unknown uniform type: " + String.valueOf((Object)this));
            }
            Config.warn("Invalid index2, parameter: " + String.valueOf((Object)this) + ", index: " + index2);
            return 0.0f;
        }
        Config.warn("Invalid index1, parameter: " + String.valueOf((Object)this) + ", index: " + index1);
        return 0.0f;
    }
