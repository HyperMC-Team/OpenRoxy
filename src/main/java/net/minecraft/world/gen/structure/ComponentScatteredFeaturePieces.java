...
public class ComponentScatteredFeaturePieces {
    ...
    extends Feature {
        ...
        public boolean addComponentParts(World worldIn, Random randomIn, StructureBoundingBox structureBoundingBoxIn) {
            ...
            if (!this.hasWitch && structureBoundingBoxIn.isVecInside(new BlockPos(l1 = this.getXWithOffset(2, 5), i2 = this.getYWithOffset(2), k1 = this.getZWithOffset(2, 5)))) {
                this.hasWitch = true;
                EntityWitch entitywitch = new EntityWitch(worldIn);
                entitywitch.setLocationAndAngles((double)l1 + 0.5, i2, (double)k1 + 0.5, 0.0f, 0.0f);
        ...
        protected boolean func_74935_a(World worldIn, StructureBoundingBox p_74935_2_, int p_74935_3_) {
            ...
            for (int k = this.boundingBox.minZ; k <= this.boundingBox.maxZ; ++k) {
                for (int l = this.boundingBox.minX; l <= this.boundingBox.maxX; ++l) {
                    ...
                    if (!p_74935_2_.isVecInside(blockpos$mutableblockpos)) continue;
