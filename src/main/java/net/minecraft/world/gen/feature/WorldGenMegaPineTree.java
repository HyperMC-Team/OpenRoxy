...
extends WorldGenHugeTrees {
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if ((block = worldIn.getBlockState(position.add(1, j, 1)).getBlock()).getMaterial() == Material.air || block.getMaterial() == Material.leaves) {
                this.setBlockAndNotifyAdequately(worldIn, position.add(1, j, 1), this.woodMetadata);
            }
            if ((block = worldIn.getBlockState(position.add(0, j, 1)).getBlock()).getMaterial() != Material.air && block.getMaterial() != Material.leaves) continue;
    ...
    private void func_175934_c(World worldIn, BlockPos p_175934_2_) {
        for (int i = 2; i >= -3; --i) {
            ...
            if (block.getMaterial() != Material.air && i < 0) break;
        }
    }
}
