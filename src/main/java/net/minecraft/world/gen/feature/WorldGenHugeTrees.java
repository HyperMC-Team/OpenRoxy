            ...
            for (int i = 0; i <= 1 + p_175926_3_; ++i) {
                ...
                for (int k = -j; k <= j && flag; ++k) {
                    for (int l = -j; l <= j && flag; ++l) {
                        if (p_175926_2_.getY() + i >= 0 && p_175926_2_.getY() + i < 256 && this.func_150523_a(worldIn.getBlockState(p_175926_2_.add(k, i, l)).getBlock())) continue;
                        flag = false;
                    }
                }
            }
    ...
    protected void func_175925_a(World worldIn, BlockPos p_175925_2_, int p_175925_3_) {
        ...
        for (int j = -p_175925_3_; j <= p_175925_3_ + 1; ++j) {
            for (int k = -p_175925_3_; k <= p_175925_3_ + 1; ++k) {
                ...
                int l = j - 1;
                int i1 = k - 1;
                if (j * j + k * k > i && l * l + i1 * i1 > i && j * j + i1 * i1 > i && l * l + k * k > i || (material = worldIn.getBlockState(blockpos = p_175925_2_.add(j, 0, k)).getBlock().getMaterial()) != Material.air && material != Material.leaves) continue;
    ...
    protected void func_175928_b(World worldIn, BlockPos p_175928_2_, int p_175928_3_) {
        ...
        for (int j = -p_175928_3_; j <= p_175928_3_; ++j) {
            for (int k = -p_175928_3_; k <= p_175928_3_; ++k) {
                BlockPos blockpos;
                Material material;
                if (j * j + k * k > i || (material = worldIn.getBlockState(blockpos = p_175928_2_.add(j, 0, k)).getBlock().getMaterial()) != Material.air && material != Material.leaves) continue;
                this.setBlockAndNotifyAdequately(worldIn, blockpos, this.leavesMetadata);
            }
        }
    }
