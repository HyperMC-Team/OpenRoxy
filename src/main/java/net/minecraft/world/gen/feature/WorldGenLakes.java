        ...
        for (int k1 = 0; k1 < 16; ++k1) {
            for (int l2 = 0; l2 < 16; ++l2) {
                for (int k = 0; k < 8; ++k) {
                    ...
                    if (k >= 4 || material.isSolid() || worldIn.getBlockState(position.add(k1, k, l2)).getBlock() == this.block) continue;
                    return false;
                }
            }
        }
        ...
        for (int i2 = 0; i2 < 16; ++i2) {
            for (int j3 = 0; j3 < 16; ++j3) {
                for (int j4 = 4; j4 < 8; ++j4) {
                    BlockPos blockpos;
                    if (!aboolean[(i2 * 16 + j3) * 8 + j4] || worldIn.getBlockState(blockpos = position.add(i2, j4 - 1, j3)).getBlock() != Blocks.dirt || worldIn.getLightFor(EnumSkyBlock.SKY, position.add(i2, j4, j3)) <= 0) continue;
