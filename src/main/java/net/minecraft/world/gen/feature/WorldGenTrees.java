...
extends WorldGenAbstractTree {
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= 256) {
            for (int j = position.getY(); j <= position.getY() + 1 + i; ++j) {
                ...
                for (int l = position.getX() - k; l <= position.getX() + k && flag; ++l) {
                    for (int i1 = position.getZ() - k; i1 <= position.getZ() + k && flag; ++i1) {
                        if (j >= 0 && j < 256) {
                            if (this.func_150523_a(worldIn.getBlockState(blockpos$mutableblockpos.set(l, j, i1)).getBlock())) continue;
                            flag = false;
                            continue;
                        }
                        flag = false;
            ...
            if ((block1 == Blocks.grass || block1 == Blocks.dirt || block1 == Blocks.farmland) && position.getY() < 256 - i - 1) {
                ...
                for (int i3 = position.getY() - k2 + i; i3 <= position.getY() + i; ++i3) {
                    ...
                    for (int k1 = position.getX() - j1; k1 <= position.getX() + j1; ++k1) {
                        ...
                        for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
                            ...
                            if (Math.abs(l1) == j1 && Math.abs(j2) == j1 && (rand.nextInt(2) == 0 || i4 == 0) || (block = worldIn.getBlockState(blockpos = new BlockPos(k1, i3, i2)).getBlock()).getMaterial() != Material.air && block.getMaterial() != Material.leaves && block.getMaterial() != Material.vine) continue;
                ...
                for (int j3 = 0; j3 < i; ++j3) {
                    ...
                    if (block2.getMaterial() != Material.air && block2.getMaterial() != Material.leaves && block2.getMaterial() != Material.vine) continue;
                    ...
                    for (int k3 = position.getY() - 3 + i; k3 <= position.getY() + i; ++k3) {
                        ...
                        for (int l4 = position.getX() - k4; l4 <= position.getX() + k4; ++l4) {
                            for (int i5 = position.getZ() - k4; i5 <= position.getZ() + k4; ++i5) {
                                ...
                                if (rand.nextInt(4) != 0 || worldIn.getBlockState(blockpos1).getBlock().getMaterial() != Material.air) continue;
    ...
    private void func_181650_b(World p_181650_1_, BlockPos p_181650_2_, PropertyBool p_181650_3_) {
        ...
        for (int i = 4; p_181650_1_.getBlockState(p_181650_2_).getBlock().getMaterial() == Material.air && i > 0; --i) {
            this.func_181651_a(p_181650_1_, p_181650_2_, p_181650_3_);
            p_181650_2_ = p_181650_2_.down();
        }
    }
