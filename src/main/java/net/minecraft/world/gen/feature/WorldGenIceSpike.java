        ...
        for (int k = 0; k < i; ++k) {
            ...
            for (int i1 = -l; i1 <= l; ++i1) {
                ...
                for (int j1 = -l; j1 <= l; ++j1) {
                    ...
                    if (k == 0 || l <= 1 || (block = worldIn.getBlockState(position.add(i1, -k, j1)).getBlock()).getMaterial() != Material.air && block != Blocks.dirt && block != Blocks.snow && block != Blocks.ice) continue;
                    this.setBlockAndNotifyAdequately(worldIn, position.add(i1, -k, j1), Blocks.packed_ice.getDefaultState());
                }
            }
        }
