...
public abstract class BiomeGenBase {
    ...
    protected BiomeGenBase func_150557_a(int colorIn, boolean p_150557_2_) {
        this.color = colorIn;
        this.field_150609_ah = p_150557_2_ ? (colorIn & 0xFEFEFE) >> 1 : colorIn;
        return this;
    }
        ...
        for (int j1 = 255; j1 >= 0; --j1) {
            ...
            if (j == -1) {
                ...
                if (j1 < i && (iblockstate == null || iblockstate.getBlock().getMaterial() == Material.air)) {
                    iblockstate = this.getFloatTemperature(blockpos$mutableblockpos.set(x, j1, z)) < 0.15f ? Blocks.ice.getDefaultState() : Blocks.water.getDefaultState();
                }
                j = k;
                if (j1 >= i - 1) {
