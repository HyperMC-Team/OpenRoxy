...
implements IBlockAccess {
    ...
    public BiomeGenBase getBiomeGenForCoords(final BlockPos pos) {
        if (this.isBlockLoaded(pos)) {
            ...
            try {
                ...
                crashreportcategory.addCrashSectionCallable("Class235", new Callable<String>(){
    ...
    public BlockPos getHeight(BlockPos pos) {
        int i = pos.getX() >= -30000000 && pos.getZ() >= -30000000 && pos.getX() < 30000000 && pos.getZ() < 30000000 ? (this.isChunkLoaded(pos.getX() >> 4, pos.getZ() >> 4, true) ? this.getChunkFromChunkCoords(pos.getX() >> 4, pos.getZ() >> 4).getHeightValue(pos.getX() & 0xF, pos.getZ() & 0xF) : 0) : this.getSeaLevel() + 1;
    ...
    public List<AxisAlignedBB> getCollidingBoundingBoxes(Entity entityIn, AxisAlignedBB bb) {
        ...
        for (int k1 = i; k1 < j; ++k1) {
            for (int l1 = i1; l1 < j1; ++l1) {
                if (!this.isBlockLoaded(blockpos$mutableblockpos.set(k1, 64, l1))) continue;
        ...
        for (int j2 = 0; j2 < list1.size(); ++j2) {
            if (entityIn.riddenByEntity == list1 || entityIn.ridingEntity == list1) continue;
            ...
            if ((axisalignedbb = entityIn.getCollisionBox(list1.get(j2))) == null || !axisalignedbb.intersectsWith(bb)) continue;
            list.add(axisalignedbb);
        }
        return list;
    }
    ...
    public BlockPos getTopSolidOrLiquidBlock(BlockPos pos) {
        ...
        while (!(blockpos.getY() < 0 || (material = chunk.getBlock(blockpos1 = blockpos.down()).getMaterial()).blocksMovement() && material != Material.leaves)) {
            blockpos = blockpos1;
        }
        return blockpos;
    }
    ...
    public void updateEntityWithOptionalForce(Entity entityIn, boolean forceUpdate) {
        ...
        if (!forceUpdate || this.isAreaLoaded(i - k, 0, j - k, i + k, 0, j + k, true)) {
            ...
            if (forceUpdate && entityIn.addedToChunk) {
                ...
                if (this.skiptick > 0 && entityIn == Minecraft.getMinecraft().thePlayer) {
                    --this.skiptick;
                    Class13.Method7(new Class50());
                    Class115.mc.thePlayer.onUpdateWalkingPlayer();
                } else if (entityIn.ridingEntity != null) {
    ...
    public boolean checkNoEntityCollision(AxisAlignedBB bb, Entity entityIn) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (entity.isDead || !entity.preventEntitySpawning || entity == entityIn || entityIn != null && (entityIn.ridingEntity == entity || entityIn.riddenByEntity == entity)) continue;
            return false;
        }
        return true;
    }
    ...
    public boolean isMaterialInBB(AxisAlignedBB bb, Material materialIn) {
        ...
        for (int k1 = i; k1 < j; ++k1) {
            for (int l1 = k; l1 < l; ++l1) {
                for (int i2 = i1; i2 < j1; ++i2) {
                    if (this.getBlockState(blockpos$mutableblockpos.set(k1, l1, i2)).getBlock().getMaterial() != materialIn) continue;
                    return true;
                }
            }
        }
    ...
    public TileEntity getTileEntity(BlockPos pos) {
        ...
        if (tileentity == null) {
            for (int j = 0; j < this.addedTileEntityList.size(); ++j) {
                ...
                if (tileentity2.isInvalid() || !tileentity2.getPos().equals(pos)) continue;
    ...
    public EntityPlayer getClosestPlayer(double x, double y, double z, double distance) {
        ...
        for (int i = 0; i < this.playerEntities.size(); ++i) {
            ...
            if (!EntitySelectors.NOT_SPECTATING.apply(entityplayer1)) continue;
    ...
    public void playAuxSFXAtEntity(EntityPlayer player, int sfxType, BlockPos pos, int p_180498_4_) {
        try {
            ...
            CrashReport crashreport = CrashReport.makeCrashReport(throwable, "Playing level events");
            CrashReportCategory crashreportcategory = crashreport.makeCategory("Level events being played");
            crashreportcategory.addCrashSection("Block coordinates", CrashReportCategory.getCoordinateInfo(pos));
            crashreportcategory.addCrashSection("Class7 source", player);
            crashreportcategory.addCrashSection("Class7 type", sfxType);
            crashreportcategory.addCrashSection("Class7 data", p_180498_4_);
            throw new ReportedException(crashreport);
        }
    }
