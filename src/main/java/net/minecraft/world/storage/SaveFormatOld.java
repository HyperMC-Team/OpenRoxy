...
implements ISaveFormat {
    ...
    public WorldInfo getWorldInfo(String saveName) {
        ...
        if (file2.exists()) {
            try {
                ...
                logger.error("Exception reading " + String.valueOf(file2), (Throwable)exception1);
            }
        }
        if ((file2 = new File(file1, "level.dat_old")).exists()) {
            try {
                ...
                logger.error("Exception reading " + String.valueOf(file2), (Throwable)exception);
            }
        }
        return null;
    }
    ...
    protected static boolean deleteFiles(File[] files) {
        for (int i = 0; i < files.length; ++i) {
            File file1 = files[i];
            logger.debug("Deleting " + String.valueOf(file1));
            if (file1.isDirectory() && !SaveFormatOld.deleteFiles(file1.listFiles())) {
                logger.warn("Couldn't delete directory " + String.valueOf(file1));
                return false;
            }
            if (file1.delete()) continue;
            logger.warn("Couldn't delete file " + String.valueOf(file1));
            return false;
        }
        return true;
    }
