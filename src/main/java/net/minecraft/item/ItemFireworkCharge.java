...
extends Item {
    ...
    public int getColorFromItemStack(ItemStack stack, int renderPass) {
        ...
        NBTBase nbtbase = ItemFireworkCharge.getExplosionTag(stack, "Class223");
    ...
    public static void addExplosionInfo(NBTTagCompound nbt, List<String> tooltip) {
        ...
        int[] aint = nbt.getIntArray("Class223");
        if (aint.length > 0) {
            boolean flag = true;
            Object s = "";
            for (int i : aint) {
                ...
                for (int j = 0; j < ItemDye.dyeColors.length; ++j) {
                    ...
                    flag1 = true;
                    s = (String)s + StatCollector.translateToLocal("item.fireworksCharge." + EnumDyeColor.byDyeDamage(j).getUnlocalizedName());
                    break;
                }
                if (flag1) continue;
                s = (String)s + StatCollector.translateToLocal("item.fireworksCharge.customColor");
            }
            tooltip.add((String)s);
        }
