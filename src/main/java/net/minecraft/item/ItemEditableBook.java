    ...
    public void addInformation(ItemStack stack, EntityPlayer playerIn, List<String> tooltip, boolean advanced) {
        if (stack.hasTagCompound()) {
            ...
            if (!StringUtils.isNullOrEmpty(s)) {
                tooltip.add(String.valueOf((Object)EnumChatFormatting.GRAY) + StatCollector.translateToLocalFormatted("book.byAuthor", s));
            }
            tooltip.add(String.valueOf((Object)EnumChatFormatting.GRAY) + StatCollector.translateToLocal("book.generation." + nbttagcompound.getInteger("generation")));
        }
    }
        ...
        if (stack != null && stack.getTagCompound() != null && !(nbttagcompound = stack.getTagCompound()).getBoolean("resolved")) {
            ...
            if (ItemEditableBook.validBookTagContents(nbttagcompound)) {
                ...
                if (player instanceof EntityPlayerMP && player.getCurrentEquippedItem() == stack) {
                    ...
                    ((EntityPlayerMP)player).playerNetServerHandler.sendPacket(new Class378(0, slot.slotNumber, stack));
                }
            }
        }
    }
