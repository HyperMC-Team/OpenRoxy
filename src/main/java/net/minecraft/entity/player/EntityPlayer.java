...
extends EntityLivingBase {
    ...
    public final ArrayList<Class307.Point> points = new ArrayList();
    ...
    private Collection<ScoreObjective> func_175137_e(Entity p_175137_1_) {
        ...
        if ((scoreplayerteam1 = this.getWorldScoreboard().getPlayersTeam(p_175137_1_.getName())) != null && (j = scoreplayerteam1.getChatFormat().getColorIndex()) >= 0 && j < IScoreObjectiveCriteria.field_178792_h.length) {
    ...
    public boolean interactWith(Entity targetEntity) {
        ...
        if (this.isSpectator()) {
            if (targetEntity instanceof IInventory) {
                this.displayGUIChest((IInventory)((Object)targetEntity));
            }
            return false;
        }
    ...
    public void attackTargetEntityWithCurrentItem(Entity targetEntity) {
        if (targetEntity.canAttackWithItem() && !targetEntity.hitByEntity(this)) {
            ...
            int i = 0;
            float f1 = 0.0f;
            f1 = targetEntity instanceof EntityLivingBase ? EnchantmentHelper.getModifierForCreature(this.getHeldItem(), ((EntityLivingBase)targetEntity).getCreatureAttribute()) : EnchantmentHelper.getModifierForCreature(this.getHeldItem(), EnumCreatureAttribute.UNDEFINED);
            ...
            if (f > 0.0f || f1 > 0.0f) {
                ...
                if (flag2) {
                    ...
                    if (i > 0) {
                        targetEntity.addVelocity(-MathHelper.sin(this.movementYaw * (float)Math.PI / 180.0f) * (float)i * 0.5f, 0.1, MathHelper.cos(this.movementYaw * (float)Math.PI / 180.0f) * (float)i * 0.5f);
                        this.motionX *= 0.6;
                        this.motionZ *= 0.6;
                        this.setSprinting(false);
                    }
                    if (targetEntity instanceof EntityPlayerMP && targetEntity.velocityChanged) {
                        ((EntityPlayerMP)targetEntity).playerNetServerHandler.sendPacket(new Class349(targetEntity));
