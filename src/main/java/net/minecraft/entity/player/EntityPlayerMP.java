...
implements ICrafting {
    ...
    public void sendEnterCombat() {
        ...
        this.playerNetServerHandler.sendPacket(new Class422(this.getCombatTracker(), Class422.Event.ENTER_COMBAT));
    }

    @Override
    public void sendEndCombat() {
        super.sendEndCombat();
        this.playerNetServerHandler.sendPacket(new Class422(this.getCombatTracker(), Class422.Event.END_COMBAT));
    }
    ...
    public void onUpdate() {
        ...
        while (!this.destroyedItemsNetCache.isEmpty()) {
            ...
            this.playerNetServerHandler.sendPacket(new Class402(aint));
        }
        if (!this.loadedChunks.isEmpty()) {
            ...
            if (!list.isEmpty()) {
                if (list.size() == 1) {
                    this.playerNetServerHandler.sendPacket(new Class386((Chunk)list.get(0), true, 65535));
                } else {
                    this.playerNetServerHandler.sendPacket(new Class396(list));
                }
    ...
    public void onUpdateEntity() {
        try {
            ...
            if (this.getHealth() != this.lastHealth || this.lastFoodLevel != this.foodStats.getFoodLevel() || this.foodStats.getSaturationLevel() == 0.0f != this.wasHungry) {
                this.playerNetServerHandler.sendPacket(new Class398(this.getHealth(), this.foodStats.getFoodLevel(), this.foodStats.getSaturationLevel()));
            ...
            if (this.experienceTotal != this.lastExperience) {
                ...
                this.playerNetServerHandler.sendPacket(new Class344(this.experience, this.experienceTotal, this.experienceLevel));
            }
    ...
    protected void updateBiomesExplored() {
        ...
        if (this.getStatFile().canUnlockAchievement(AchievementList.exploreAllBiomes) && jsonserializableset.size() >= BiomeGenBase.explorationBiomesList.size()) {
            HashSet set = Sets.newHashSet(BiomeGenBase.explorationBiomesList);
            Iterator iterator = jsonserializableset.iterator();
            while (iterator.hasNext()) {
                String s1 = (String)iterator.next();
                Iterator iterator2 = set.iterator();
                while (iterator2.hasNext()) {
                    BiomeGenBase biomegenbase1 = (BiomeGenBase)iterator2.next();
    ...
    public void travelToDimension(int dimensionId) {
        if (this.dimension == 1 && dimensionId == 1) {
            ...
            this.playerNetServerHandler.sendPacket(new Class342(4, 0.0f));
        } else {
    ...
    private void sendTileEntityUpdate(TileEntity p_147097_1_) {
        Packet packet;
        if (p_147097_1_ != null && (packet = p_147097_1_.getDescriptionPacket()) != null) {
            this.playerNetServerHandler.sendPacket(packet);
        }
    }
    ...
    public EntityPlayer.EnumStatus trySleep(BlockPos bedLocation) {
        ...
        if (entityplayer$enumstatus == EntityPlayer.EnumStatus.OK) {
            Class338 packet = new Class338(this, bedLocation);
    ...
    public void wakeUpPlayer(boolean immediately, boolean updateWorldFlag, boolean setSpawn) {
        if (this.isPlayerSleeping()) {
            this.getServerForPlayer().getEntityTracker().func_151248_b(this, new Class367(this, 2));
        }
    ...
    public void mountEntity(Entity entityIn) {
        ...
        if (entityIn != entity) {
            this.playerNetServerHandler.sendPacket(new Class370(0, this, this.ridingEntity));
            this.playerNetServerHandler.setPlayerLocation(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch);
        }
    }
    ...
    public void openEditSign(TileEntitySign signTile) {
        ...
        this.playerNetServerHandler.sendPacket(new Class408(signTile.getPos()));
    }
    ...
    public void displayGui(IInteractionObject guiOwner) {
        ...
        this.playerNetServerHandler.sendPacket(new Class403(this.currentWindowId, guiOwner.getGuiID(), guiOwner.getDisplayName()));
    ...
    public void displayGUIChest(IInventory chestInventory) {
        ...
        if (chestInventory instanceof ILockableContainer && (ilockablecontainer = (ILockableContainer)chestInventory).isLocked() && !this.canOpen(ilockablecontainer.getLockCode()) && !this.isSpectator()) {
            this.playerNetServerHandler.sendPacket(new Class420(new ChatComponentTranslation("container.isLocked", chestInventory.getDisplayName()), (byte) 2));
            this.playerNetServerHandler.sendPacket(new Class389("random.door_close", this.posX, this.posY, this.posZ, 1.0f, 1.0f));
            return;
        }
        this.getNextWindowId();
        if (chestInventory instanceof IInteractionObject) {
            this.playerNetServerHandler.sendPacket(new Class403(this.currentWindowId, ((IInteractionObject)((Object)chestInventory)).getGuiID(), chestInventory.getDisplayName(), chestInventory.getSizeInventory()));
            this.openContainer = ((IInteractionObject)((Object)chestInventory)).createContainer(this.inventory, this);
        } else {
            this.playerNetServerHandler.sendPacket(new Class403(this.currentWindowId, "minecraft:container", chestInventory.getDisplayName(), chestInventory.getSizeInventory()));
    ...
    public void displayVillagerTradeGui(IMerchant villager) {
        ...
        this.playerNetServerHandler.sendPacket(new Class403(this.currentWindowId, "minecraft:villager", ichatcomponent, iinventory.getSizeInventory()));
        MerchantRecipeList merchantrecipelist = villager.getRecipes(this);
        if (merchantrecipelist != null) {
            PacketBuffer packetbuffer = new PacketBuffer(Unpooled.buffer());
            packetbuffer.writeInt(this.currentWindowId);
            merchantrecipelist.writeToBuf(packetbuffer);
            this.playerNetServerHandler.sendPacket(new Class377("MC|TrList", packetbuffer));
        }
    }
    ...
    public void displayGUIHorse(EntityHorse horse, IInventory horseInventory) {
        ...
        this.getNextWindowId();
        this.playerNetServerHandler.sendPacket(new Class403(this.currentWindowId, "EntityHorse", horseInventory.getDisplayName(), horseInventory.getSizeInventory(), horse.getEntityId()));
    ...
    public void displayGUIBook(ItemStack bookStack) {
        ...
        if (item == Items.written_book) {
            this.playerNetServerHandler.sendPacket(new Class377("MC|BOpen", new PacketBuffer(Unpooled.buffer())));
        }
    }
    ...
    public void sendSlotContents(Container containerToSend, int slotInd, ItemStack stack) {
        if (!(containerToSend.getSlot(slotInd) instanceof SlotCrafting) && !this.isChangingQuantityOnly) {
            this.playerNetServerHandler.sendPacket(new Class378(containerToSend.windowId, slotInd, stack));
        }
    }
    ...
    public void updateCraftingInventory(Container containerToSend, List<ItemStack> itemsList) {
        this.playerNetServerHandler.sendPacket(new Class393(containerToSend.windowId, itemsList));
        this.playerNetServerHandler.sendPacket(new Class378(-1, -1, this.inventory.getItemStack()));
    }

    @Override
    public void sendProgressBarUpdate(Container containerIn, int varToUpdate, int newValue) {
        this.playerNetServerHandler.sendPacket(new Class390(containerIn.windowId, varToUpdate, newValue));
    }

    @Override
    public void sendAllWindowProperties(Container p_175173_1_, IInventory p_175173_2_) {
        for (int i = 0; i < p_175173_2_.getFieldCount(); ++i) {
            this.playerNetServerHandler.sendPacket(new Class390(p_175173_1_.windowId, i, p_175173_2_.getField(i)));
        }
    }

    @Override
    public void closeScreen() {
        this.playerNetServerHandler.sendPacket(new Class340(this.openContainer.windowId));
        this.closeContainer();
    }
    ...
    public void addChatComponentMessage(IChatComponent chatComponent) {
        this.playerNetServerHandler.sendPacket(new Class420(chatComponent));
    }

    @Override
    protected void onItemUseFinish() {
        this.playerNetServerHandler.sendPacket(new Class415(this, (byte) 9));
    ...
    public void setItemInUse(ItemStack stack, int duration) {
        ...
        if (stack != null && stack.getItem() != null && stack.getItem().getItemUseAction(stack) == EnumAction.EAT) {
            this.getServerForPlayer().getEntityTracker().func_151248_b(this, new Class367(this, 3));
        }
    }
    ...
    protected void onNewPotionEffect(PotionEffect id) {
        ...
        this.playerNetServerHandler.sendPacket(new Class347(this.getEntityId(), id));
    }
    ...
    protected void onFinishedPotionEffect(PotionEffect effect) {
        ...
        this.playerNetServerHandler.sendPacket(new Class407(this.getEntityId(), effect));
    }
    ...
    public void onCriticalHit(Entity entityHit) {
        this.getServerForPlayer().getEntityTracker().func_151248_b(this, new Class367(entityHit, 4));
    }

    @Override
    public void onEnchantmentCritical(Entity entityHit) {
        this.getServerForPlayer().getEntityTracker().func_151248_b(this, new Class367(entityHit, 5));
    }

    @Override
    public void sendPlayerAbilities() {
        if (this.playerNetServerHandler != null) {
            this.playerNetServerHandler.sendPacket(new Class417(this.capabilities));
    ...
    public void setGameType(WorldSettings.GameType gameType) {
        ...
        this.playerNetServerHandler.sendPacket(new Class342(3, gameType.getID()));
    ...
    public void addChatMessage(IChatComponent component) {
        this.playerNetServerHandler.sendPacket(new Class420(component));
    }
    ...
    public void handleClientSettings(Class356 packetIn) {
    ...
    public void loadResourcePack(String url, String hash) {
        this.playerNetServerHandler.sendPacket(new Class423(url, hash));
    }
    ...
    public void removeEntity(Entity p_152339_1_) {
        if (p_152339_1_ instanceof EntityPlayer) {
            this.playerNetServerHandler.sendPacket(new Class402(p_152339_1_.getEntityId()));
        } else {
    ...
    public void setSpectatingEntity(Entity entityToSpectate) {
        ...
        if (entity != this.spectatingEntity) {
            this.playerNetServerHandler.sendPacket(new Class410(this.spectatingEntity));
