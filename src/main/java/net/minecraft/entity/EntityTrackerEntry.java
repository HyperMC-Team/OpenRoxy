...
public class EntityTrackerEntry {
    ...
    public void updatePlayerList(List<EntityPlayer> players) {
        ...
        if (this.field_85178_v != this.trackedEntity.ridingEntity || this.trackedEntity.ridingEntity != null && this.updateCounter % 60 == 0) {
            ...
            this.sendPacketToTrackedPlayers(new Class370(0, this.trackedEntity, this.trackedEntity.ridingEntity));
        }
        ...
        if (this.updateCounter % this.updateFrequency == 0 || this.trackedEntity.isAirBorne || this.trackedEntity.getDataWatcher().hasObjectChanged()) {
            if (this.trackedEntity.ridingEntity == null) {
                ...
                if (this.updateCounter > 0 || this.trackedEntity instanceof EntityArrow) {
                    if (j2 >= -128 && j2 < 128 && k2 >= -128 && k2 < 128 && i >= -128 && i < 128 && this.ticksSinceLastForcedTeleport <= 400 && !this.ridingEntity && this.onGround == this.trackedEntity.onGround) {
                        if (!(flag && flag1 || this.trackedEntity instanceof EntityArrow)) {
                            if (flag) {
                                packet1 = new Class397.S15PacketEntityRelMove(this.trackedEntity.getEntityId(), (byte)j2, (byte)k2, (byte)i, this.trackedEntity.onGround);
                            } else if (flag1) {
                                packet1 = new Class397.S16PacketEntityLook(this.trackedEntity.getEntityId(), (byte)l1, (byte)i2, this.trackedEntity.onGround);
                            }
                        } else {
                            packet1 = new Class397.S17PacketEntityLookMove(this.trackedEntity.getEntityId(), (byte)j2, (byte)k2, (byte)i, (byte)l1, (byte)i2, this.trackedEntity.onGround);
                        }
                    } else {
                        this.onGround = this.trackedEntity.onGround;
                        this.ticksSinceLastForcedTeleport = 0;
                        packet1 = new Class406(this.trackedEntity.getEntityId(), k, j1, k1, (byte)l1, (byte)i2, this.trackedEntity.onGround);
                    }
                }
                if (this.sendVelocityUpdates && ((d4 = (d0 = this.trackedEntity.motionX - this.lastTrackedEntityMotionX) * d0 + (d1 = this.trackedEntity.motionY - this.lastTrackedEntityMotionY) * d1 + (d2 = this.trackedEntity.motionZ - this.motionZ) * d2) > (d3 = 0.02) * d3 || d4 > 0.0 && this.trackedEntity.motionX == 0.0 && this.trackedEntity.motionY == 0.0 && this.trackedEntity.motionZ == 0.0)) {
                    ...
                    this.sendPacketToTrackedPlayers(new Class349(this.trackedEntity.getEntityId(), this.lastTrackedEntityMotionX, this.lastTrackedEntityMotionY, this.motionZ));
                }
                if (packet1 != null) {
                ...
                if (flag2) {
                    this.sendPacketToTrackedPlayers(new Class397.S16PacketEntityLook(this.trackedEntity.getEntityId(), (byte)j, (byte)i1, this.trackedEntity.onGround));
            ...
            if (Math.abs(l - this.lastHeadMotion) >= 4) {
                this.sendPacketToTrackedPlayers(new Class404(this.trackedEntity, (byte)l));
        ...
        if (this.trackedEntity.velocityChanged) {
            this.func_151261_b(new Class349(this.trackedEntity));
    ...
    private void sendMetadataToAllAssociatedPlayers() {
        ...
        if (datawatcher.hasObjectChanged()) {
            this.func_151261_b(new Class419(this.trackedEntity.getEntityId(), datawatcher, false));
        }
        if (this.trackedEntity instanceof EntityLivingBase) {
            ...
            if (!set.isEmpty()) {
                this.func_151261_b(new Class388(this.trackedEntity.getEntityId(), set));
            }
            set.clear();
        }
    }
            ...
            if (this.func_180233_c(playerMP)) {
                if (!this.trackingPlayers.contains(playerMP) && (this.isPlayerWatchingThisChunk(playerMP) || this.trackedEntity.forceSpawn)) {
                    ...
                    if (!this.trackedEntity.getDataWatcher().getIsBlank()) {
                        playerMP.playerNetServerHandler.sendPacket(new Class419(this.trackedEntity.getEntityId(), this.trackedEntity.getDataWatcher(), true));
                    }
                    if ((nbttagcompound = this.trackedEntity.getNBTTagCompound()) != null) {
                        playerMP.playerNetServerHandler.sendPacket(new Class424(this.trackedEntity.getEntityId(), nbttagcompound));
                    }
                    if (this.trackedEntity instanceof EntityLivingBase && !(collection = (serversideattributemap = (ServersideAttributeMap)((EntityLivingBase)this.trackedEntity).getAttributeMap()).getWatchedAttributes()).isEmpty()) {
                        playerMP.playerNetServerHandler.sendPacket(new Class388(this.trackedEntity.getEntityId(), collection));
                    }
                    this.lastTrackedEntityMotionX = this.trackedEntity.motionX;
                    this.lastTrackedEntityMotionY = this.trackedEntity.motionY;
                    this.motionZ = this.trackedEntity.motionZ;
                    if (this.sendVelocityUpdates && !(packet instanceof Class345)) {
                        playerMP.playerNetServerHandler.sendPacket(new Class349(this.trackedEntity.getEntityId(), this.trackedEntity.motionX, this.trackedEntity.motionY, this.trackedEntity.motionZ));
                    }
                    if (this.trackedEntity.ridingEntity != null) {
                        playerMP.playerNetServerHandler.sendPacket(new Class370(0, this.trackedEntity, this.trackedEntity.ridingEntity));
                    }
                    if (this.trackedEntity instanceof EntityLiving && ((EntityLiving)this.trackedEntity).getLeashedToEntity() != null) {
                        playerMP.playerNetServerHandler.sendPacket(new Class370(1, this.trackedEntity, ((EntityLiving)this.trackedEntity).getLeashedToEntity()));
                    }
                    if (this.trackedEntity instanceof EntityLivingBase) {
                        for (int i = 0; i < 5; ++i) {
                            ItemStack itemstack = ((EntityLivingBase)this.trackedEntity).getEquipmentInSlot(i);
                            if (itemstack == null) continue;
                            playerMP.playerNetServerHandler.sendPacket(new Class376(this.trackedEntity.getEntityId(), i, itemstack));
                        }
                    }
                    if (this.trackedEntity instanceof EntityPlayer && (entityplayer = (EntityPlayer)this.trackedEntity).isPlayerSleeping()) {
                        playerMP.playerNetServerHandler.sendPacket(new Class338(entityplayer, new BlockPos(this.trackedEntity)));
                    }
                    if (this.trackedEntity instanceof EntityLivingBase) {
                        ...
                        for (PotionEffect potioneffect : entitylivingbase.getActivePotionEffects()) {
                            playerMP.playerNetServerHandler.sendPacket(new Class347(this.trackedEntity.getEntityId(), potioneffect));
                        }
                    }
                }
    ...
    private Packet createSpawnPacket() {
        ...
        if (this.trackedEntity instanceof EntityItem) {
            return new Class339(this.trackedEntity, 2, 1);
        }
        if (this.trackedEntity instanceof EntityPlayerMP) {
            return new Class368((EntityPlayer)this.trackedEntity);
        }
        if (this.trackedEntity instanceof EntityMinecart) {
            EntityMinecart entityminecart = (EntityMinecart)this.trackedEntity;
            return new Class339(this.trackedEntity, 10, entityminecart.getMinecartType().getNetworkID());
        }
        if (this.trackedEntity instanceof EntityBoat) {
            return new Class339(this.trackedEntity, 1);
        }
        ...
        if (this.trackedEntity instanceof EntityFireball) {
            ...
            s0epacketspawnobject2 = entityfireball.shootingEntity != null ? new Class339(this.trackedEntity, i, ((EntityFireball)this.trackedEntity).shootingEntity.getEntityId()) : new Class339(this.trackedEntity, i, 0);
        ...
        if (this.trackedEntity instanceof EntityEgg) {
            return new Class339(this.trackedEntity, 62);
        }
        if (this.trackedEntity instanceof EntityTNTPrimed) {
            return new Class339(this.trackedEntity, 50);
        }
        if (this.trackedEntity instanceof EntityEnderCrystal) {
            return new Class339(this.trackedEntity, 51);
        }
        if (this.trackedEntity instanceof EntityFallingBlock) {
            ...
            return new Class339(this.trackedEntity, 70, Block.getStateId(entityfallingblock.getBlock()));
        }
        if (this.trackedEntity instanceof EntityArmorStand) {
            return new Class339(this.trackedEntity, 78);
        }
        if (this.trackedEntity instanceof EntityPainting) {
            return new Class418((EntityPainting)this.trackedEntity);
        }
        if (this.trackedEntity instanceof EntityItemFrame) {
            EntityItemFrame entityitemframe = (EntityItemFrame)this.trackedEntity;
            Class339 s0epacketspawnobject1 = new Class339(this.trackedEntity, 71, entityitemframe.facingDirection.getHorizontalIndex());
        ...
        if (this.trackedEntity instanceof EntityLeashKnot) {
            ...
            Class339 s0epacketspawnobject = new Class339(this.trackedEntity, 77);
        ...
        if (this.trackedEntity instanceof EntityXPOrb) {
            return new Class373((EntityXPOrb)this.trackedEntity);
        }
        throw new IllegalArgumentException("Don't know how to add " + String.valueOf(this.trackedEntity.getClass()) + "!");
    }
