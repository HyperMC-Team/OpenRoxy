    ...
    public void onLivingUpdate() {
        ...
        for (int j = 0; j < 2; ++j) {
            ...
            if (entity1 != null) {
                ...
                float f = (float)(MathHelper.atan2(d8, d6) * 180.0 / Math.PI) - 90.0f;
                float f1 = (float)(-(MathHelper.atan2(d7, d9) * 180.0 / Math.PI));
    ...
    protected void updateAITasks() {
        if (this.getInvulTime() > 0) {
            ...
            block0: for (int i = 1; i < 3; ++i) {
                ...
                List<EntityLivingBase> list = this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().expand(20.0, 8.0, 20.0), Predicates.and(attackEntitySelector, EntitySelectors.NOT_SPECTATING));
                    ...
                    for (int k2 = -1; k2 <= 1; ++k2) {
                        for (int l2 = -1; l2 <= 1; ++l2) {
                            for (int j = 0; j <= 3; ++j) {
                                ...
                                if (block.getMaterial() == Material.air || !EntityWither.canDestroyBlock(block)) continue;
