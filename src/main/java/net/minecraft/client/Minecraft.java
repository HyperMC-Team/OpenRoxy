...
import Rename.Class131;
import Rename.Class138;
import Rename.Class165;
import Rename.Class87;
...
import Rename.Class337.EnumState;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    public static final boolean isRunningOnMac = Util.getOSType() == EnumOS.OSX;
    ...
    public Minecraft(GameConfiguration gameConfig) {
        ...
        logger.info("Class261 user: " + this.session.getUsername());
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        Class165.playSound(new ResourceLocation("bloodline/oye.wav"), 0.9F);
        ...
        Class1.instance.onStart();
        Class138.drawScreen();
        this.guiLogin = new Class87();
    ...
    private void setWindowIcon() {
        EnumOS util$enumos = Util.getOSType();
        if (util$enumos != EnumOS.OSX) {
    ...
    private void startTimerHackThread() {
        Thread thread = new Thread("Class303 hack thread") {
            public void run() {
    ...
    public void refreshResources() {
        ...
        for(Entry resourcepackrepository$entry : this.mcResourcePackRepository.getRepositoryEntries()) {
    ...
    private void updateDisplayMode() throws LWJGLException {
        ...
        if (!set.contains(displaymode) && Util.getOSType() == EnumOS.OSX) {
    ...
    public void draw(int posX, int posY, int texU, int texV, int width, int height, int red, int green, int blue, int alpha) {
        ...
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        worldrenderer.pos((double)posX, (double)(posY + height), 0.0)
                .tex((double)((float)texU * f), (double)((float)(texV + height) * f1))
                .color(red, green, blue, alpha)
                .endVertex();
        worldrenderer.pos((double)(posX + width), (double)(posY + height), 0.0)
                .tex((double)((float)(texU + width) * f), (double)((float)(texV + height) * f1))
                .color(red, green, blue, alpha)
                .endVertex();
        worldrenderer.pos((double)(posX + width), (double)posY, 0.0)
                .tex((double)((float)(texU + width) * f), (double)((float)texV * f1))
                .color(red, green, blue, alpha)
                .endVertex();
        worldrenderer.pos((double)posX, (double)posY, 0.0)
                .tex((double)((float)texU * f), (double)((float)texV * f1))
                .color(red, green, blue, alpha)
                .endVertex();
    ...
    public void displayGuiScreen(GuiScreen guiScreenIn) {
        ...
        if (guiScreenIn == null && this.theWorld == null) {
            guiScreenIn = Class1.instance.Method3();
        } else if (guiScreenIn == null && this.thePlayer.getHealth() <= 0.0F) {
            guiScreenIn = new GuiGameOver();
        }

        if (guiScreenIn instanceof GuiMainMenu || guiScreenIn instanceof Class131) {
    ...
    private void runGameLoop() throws IOException {
        ...
        while(getSystemTime() >= this.debugUpdateTime + 1000L) {
            ...
            this.debug = String.format(
                    ...
                    (float)this.gameSettings.limitFramerate == Options.FRAMERATE_LIMIT.getValueMax() ? "inf" : this.gameSettings.limitFramerate,
    ...
    private void updateDebugProfilerName(int keyCount) {
        List<Result> list = this.mcProfiler.getProfilingData(this.debugProfilerName);
        if (list != null && !list.isEmpty()) {
            Result profiler$result = (Result)list.remove(0);
            if (keyCount == 0) {
    ...
    private void displayDebugInfo(long elapsedTicksTime) {
        if (this.mcProfiler.profilingEnabled) {
            ...
            worldrenderer.pos((double)((float)j - (float)i * 1.1F), (double)((float)k - (float)i * 0.6F - 16.0F), 0.0).color(200, 0, 0, 0).endVertex();
            worldrenderer.pos((double)((float)j - (float)i * 1.1F), (double)(k + i * 2), 0.0).color(200, 0, 0, 0).endVertex();
            worldrenderer.pos((double)((float)j + (float)i * 1.1F), (double)(k + i * 2), 0.0).color(200, 0, 0, 0).endVertex();
            worldrenderer.pos((double)((float)j + (float)i * 1.1F), (double)((float)k - (float)i * 0.6F - 16.0F), 0.0).color(200, 0, 0, 0).endVertex();
            tessellator.draw();
            ...
            for(int l = 0; l < list.size(); ++l) {
                Result profiler$result1 = (Result)list.get(l);
            ...
            for(int k2 = 0; k2 < list.size(); ++k2) {
                Result profiler$result2 = (Result)list.get(k2);
                String s1 = "";
        ...
        if (this.leftClickCounter <= 0 && !this.thePlayer.isUsingItem()) {
            if (leftClick && this.objectMouseOver != null && this.objectMouseOver.typeOfHit == MovingObjectType.BLOCK) {
                ...
                if (this.theWorld.getBlockState(blockpos).getBlock().getMaterial() != Material.air
                    ...
                    Class13.Method7(new Class43(this.objectMouseOver.getBlockPos(), this.objectMouseOver.sideHit));
    ...
    public void runTick() throws IOException {
        ...
        if (this.currentScreen != null) {
            try {
                ...
                crashreportcategory.addCrashSectionCallable("Class107 name", new Callable<String>() {
            ...
            if (this.currentScreen != null) {
                try {
                    ...
                    crashreportcategory1.addCrashSectionCallable("Class107 name", new Callable<String>() {
                        public String call() throws Exception {
                            return Minecraft.this.currentScreen.getClass().getCanonicalName();
                        }
                    });
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
                ...
                if (Keyboard.getEventKeyState()) {
                    ...
                    if (this.currentScreen != null) {
                        ...
                        if (k == 33 && Keyboard.isKeyDown(61)) {
                            this.gameSettings.setOptionValue(Options.RENDER_DISTANCE, GuiScreen.isShiftKeyDown() ? -1 : 1);
                        }
            ...
            boolean flag = this.gameSettings.chatVisibility != EnumChatVisibility.HIDDEN;

            while(this.gameSettings.keyBindInventory.isPressed()) {
                if (this.playerController.isRidingHorse()) {
                    this.thePlayer.sendHorseInventory();
                } else {
                    this.getNetHandler().addToSendQueue(new Class337(EnumState.OPEN_INVENTORY_ACHIEVEMENT));
            ...
            Class48 event = new Class48(this.thePlayer.inventory.currentItem);
    ...
    public void setDimensionAndSpawnPlayer(int dimension) {
        ...
        this.thePlayer = this.playerController
                .func_178892_a(this.theWorld, this.thePlayer == null ? new StatFileWriter() : this.thePlayer.getStatFileWriter());
    ...
    private void middleClickMouse() {
        if (this.objectMouseOver != null) {
            ...
            if (this.objectMouseOver.typeOfHit == MovingObjectType.BLOCK) {
                ...
                if (this.objectMouseOver.typeOfHit != MovingObjectType.ENTITY || this.objectMouseOver.entityHit == null || !flag) {
                    return;
                }
                        ...
                        new Callable<String>() {
                            public String call() throws Exception {
                                ...
                                return !s.equals("vanilla")
                                        ? "Definitely; Class1 brand changed to '" + s + "'"
                                        : (
        ...
        return this.thePlayer != null
                ...
                this.thePlayer.worldObj.provider instanceof WorldProviderHell
                        ...
                        this.thePlayer.worldObj.provider instanceof WorldProviderEnd
                                ? (BossStatus.bossName != null && BossStatus.statusBarTime > 0 ? MusicType.END_BOSS : MusicType.END)
                                : (this.thePlayer.capabilities.isCreativeMode && this.thePlayer.capabilities.allowFlying ? MusicType.CREATIVE : MusicType.GAME)
                )
        )
                : MusicType.MENU;
    }
