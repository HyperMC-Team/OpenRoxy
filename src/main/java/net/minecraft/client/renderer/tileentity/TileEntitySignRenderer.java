        ...
        if (TileEntitySignRenderer.isRenderText(te)) {
            ...
            if (destroyStage < 0) {
                for (int j = 0; j < te.signText.length; ++j) {
                    ...
                    Object object = s = list != null && list.size() > 0 ? list.get(0).getFormattedText() : "";
                    if (j == te.lineBeingEdited) {
                        s = "> " + (String)s + " <";
                        fontrenderer.drawString((String)s, -fontrenderer.getStringWidth((String)s) / 2, j * 10 - te.signText.length * 5, i);
                        continue;
                    }
                    fontrenderer.drawString((String)s, -fontrenderer.getStringWidth((String)s) / 2, j * 10 - te.signText.length * 5, i);
                }
