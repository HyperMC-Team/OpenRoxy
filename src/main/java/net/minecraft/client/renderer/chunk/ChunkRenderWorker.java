        ...
        try {
            if (generator.getStatus() != ChunkCompileTaskGenerator.Status.PENDING) {
                if (!generator.isFinished()) {
                    LOGGER.warn("Chunk render task was " + String.valueOf((Object)generator.getStatus()) + " when I expected it to be pending; ignoring task");
                }
                return;
            }
        ...
        if (lvt_2_1_ == null) {
            ...
            try {
                if (generator.getStatus() != ChunkCompileTaskGenerator.Status.COMPILING) {
                    if (!generator.isFinished()) {
                        LOGGER.warn("Chunk render task was " + String.valueOf((Object)generator.getStatus()) + " when I expected it to be compiling; aborting task");
                    }
            ...
            final ListenableFuture listenablefuture = Futures.allAsList((Iterable)lvt_8_1_);
                ...
                public void onSuccess(List<Object> p_onSuccess_1_) {
                    block6: {
                        ...
                        try {
                            ...
                            if (!generator.isFinished()) {
                                LOGGER.warn("Chunk render task was " + String.valueOf((Object)generator.getStatus()) + " when I expected it to be uploading; aborting task");
                            }
                        }
                        finally {
                            generator.getLock().unlock();
