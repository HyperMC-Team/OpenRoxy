...
implements IResourceManagerReloadListener {
    ...
    public RenderItem(TextureManager textureManager, ModelManager modelManager) {
        ...
        this.itemModelMesher = Reflector.ItemModelMesherForge_Constructor.exists() ? (ItemModelMesher)Reflector.newInstance(Reflector.ItemModelMesherForge_Constructor, modelManager) : new ItemModelMesher(modelManager);
        this.registerItems();
    }
    ...
    private void renderModel(IBakedModel model, int color, ItemStack stack) {
        ...
        boolean bl = flag1 = Config.isMultiTexture() && flag;
        if (flag1) {
    ...
    private void renderQuad(WorldRenderer renderer, BakedQuad quad, int color) {
        ...
        if (Reflector.IColoredBakedQuad.exists() && Reflector.IColoredBakedQuad.isInstance(quad)) {
            RenderItem.forgeHooksClient_putQuadColor(renderer, quad, color);
        } else {
    ...
    public void renderItemOverlayIntoGUI(FontRenderer fr, ItemStack stack, int xPosition, int yPosition, String text) {
        if (stack != null) {
            if (stack.stackSize != 1 || text != null) {
                Object s;
                Object object = s = text == null ? String.valueOf(stack.stackSize) : text;
                if (text == null && stack.stackSize < 1) {
                    s = String.valueOf((Object)EnumChatFormatting.RED) + String.valueOf(stack.stackSize);
                }
                GlStateManager.disableLighting();
                GlStateManager.disableDepth();
                GlStateManager.disableBlend();
                fr.drawStringWithShadow((String)s, xPosition + 19 - 2 - fr.getStringWidth((String)s), yPosition + 6 + 3, 0xFFFFFF);
                GlStateManager.enableLighting();
            ...
            if (ReflectorForge.isItemDamaged(stack)) {
                int i1;
                int j1 = (int)Math.round(13.0 - (double)stack.getItemDamage() * 13.0 / (double)stack.getMaxDamage());
                ...
                int k = i;
                int l = 0;
                if (Config.isCustomColors() && (i1 = CustomColors.getDurabilityColor(i)) >= 0) {
                    j = i1 >> 16 & 0xFF;
                    k = i1 >> 8 & 0xFF;
                    l = i1 >> 0 & 0xFF;
                }
