                ...
                for (int i = 0; i < jsonarray.size(); ++i) {
                    ...
                    if (jsonelement.isJsonPrimitive()) {
                        try {
                            ...
                            throw new JsonParseException("Invalid texture->mipmap->" + i + ": expected number, was " + String.valueOf(jsonelement), (Throwable)numberformatexception);
                        }
                    }
                    if (!jsonelement.isJsonObject()) continue;
                    throw new JsonParseException("Invalid texture->mipmap->" + i + ": expected number, was " + String.valueOf(jsonelement));
                }
            }
            catch (ClassCastException classcastexception) {
                throw new JsonParseException("Invalid texture->mipmaps: expected array, was " + String.valueOf(jsonobject.get("mipmaps")), (Throwable)classcastexception);
            }
        }
