...
import Rename.Class41;
import Rename.Class212;
...
public class PlayerControllerMP {
    ...
    public boolean clickBlock(BlockPos loc, EnumFacing face) {
        ...
        if (this.currentGameType.isCreative()) {
            this.netClientHandler.addToSendQueue(new Class365(Class365.Action.START_DESTROY_BLOCK, loc, face));
            PlayerControllerMP.clickBlockCreative(this.mc, this, loc, face);
            this.blockHitDelay = 5;
        } else if (!this.isHittingBlock || !this.isHittingPosition(loc)) {
            boolean flag;
            if (this.isHittingBlock) {
                this.netClientHandler.addToSendQueue(new Class365(Class365.Action.ABORT_DESTROY_BLOCK, this.currentBlock, face));
            }
            this.netClientHandler.addToSendQueue(new Class365(Class365.Action.START_DESTROY_BLOCK, loc, face));
    ...
    public void resetBlockRemoving() {
        if (this.isHittingBlock) {
            this.netClientHandler.addToSendQueue(new Class365(Class365.Action.ABORT_DESTROY_BLOCK, this.currentBlock, EnumFacing.DOWN));
    ...
    public boolean onPlayerDamageBlock(BlockPos posBlock, EnumFacing directionFacing) {
        ...
        if (this.currentGameType.isCreative() && this.mc.theWorld.getWorldBorder().contains(posBlock)) {
            this.blockHitDelay = 5;
            this.netClientHandler.addToSendQueue(new Class365(Class365.Action.START_DESTROY_BLOCK, posBlock, directionFacing));
        ...
        if (this.isHittingPosition(posBlock)) {
            ...
            if (this.curBlockDamageMP >= 1.0f) {
                this.isHittingBlock = false;
                this.netClientHandler.addToSendQueue(new Class365(Class365.Action.STOP_DESTROY_BLOCK, posBlock, directionFacing));
    ...
    public void syncCurrentPlayItem() {
        ...
        if (i != this.currentPlayerItem) {
            ...
            this.netClientHandler.addToSendQueue(new Class333(this.currentPlayerItem));
        }
    }
    ...
    public boolean onPlayerRightClick(EntityPlayerSP player, WorldClient worldIn, ItemStack heldStack, BlockPos hitPos, EnumFacing side, Vec3 hitVec) {
        ...
        this.netClientHandler.addToSendQueue(new Class355(hitPos, side.getIndex(), player.inventory.getCurrentItem(), f, f1, f2));
    ...
    public boolean sendUseItem(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn) {
        ...
        this.syncCurrentPlayItem();
        this.netClientHandler.addToSendQueue(new Class355(playerIn.inventory.getCurrentItem()));
    ...
    public boolean sendUseItem(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn, boolean sendNoEvent) {
        if (this.currentGameType == WorldSettings.GameType.SPECTATOR) {
            return false;
        }
        this.syncCurrentPlayItem();
        if (!sendNoEvent) {
            ...
            Class212.sendPacketNoEvent(new Class355(playerIn.inventory.getCurrentItem()));
        }
        int i = itemStackIn.stackSize;
        ItemStack itemstack = itemStackIn.useItemRightClick(worldIn, playerIn);
        if (itemstack != itemStackIn || itemstack.stackSize != i) {
    ...
    public void attackEntity(EntityPlayer playerIn, Entity targetEntity) {
        Class41 event = new Class41(targetEntity);
        ...
        this.syncCurrentPlayItem();
        this.netClientHandler.addToSendQueue(new Class334(targetEntity, Class334.Action.ATTACK));
        if (this.currentGameType != WorldSettings.GameType.SPECTATOR) {
            playerIn.attackTargetEntityWithCurrentItem(targetEntity);
        }
    }

    public void attackEntityNoSlow(Entity targetEntity) {
        Class41 event = new Class41(targetEntity);
        Class13.Method7(event);
        if (event.isCancelled()) {
            return;
    ...
    public boolean interactWithEntitySendPacket(EntityPlayer playerIn, Entity targetEntity) {
        this.syncCurrentPlayItem();
        this.netClientHandler.addToSendQueue(new Class334(targetEntity, Class334.Action.INTERACT));
    ...
    public boolean isPlayerRightClickingOnEntity(EntityPlayer player, Entity entityIn, MovingObjectPosition movingObject) {
        ...
        this.netClientHandler.addToSendQueue(new Class334(entityIn, vec3));
    ...
    public ItemStack windowClick(int windowId, int slotId, int mouseButtonClicked, int mode, EntityPlayer playerIn) {
        ...
        this.netClientHandler.addToSendQueue(new Class360(windowId, slotId, mouseButtonClicked, mode, itemstack, short1));
        return itemstack;
    }

    public void windowClickSilently(int windowId, int slotId, int mouseButtonClicked, int mode, EntityPlayer playerIn) {
        short short1 = playerIn.openContainer.getNextTransactionID(playerIn.inventory);
        this.netClientHandler.addToSendQueue(new Class360(windowId, slotId, mouseButtonClicked, mode, Minecraft.getMinecraft().thePlayer.inventory.getStackInSlot(slotId), short1));
    }

    public void sendEnchantPacket(int windowID, int button) {
        this.netClientHandler.addToSendQueue(new Class336(windowID, button));
    }

    public void sendSlotPacket(ItemStack itemStackIn, int slotId) {
        if (this.currentGameType.isCreative()) {
            this.netClientHandler.addToSendQueue(new Class343(slotId, itemStackIn));
        }
    }

    public void sendPacketDropItem(ItemStack itemStackIn) {
        if (this.currentGameType.isCreative() && itemStackIn != null) {
            this.netClientHandler.addToSendQueue(new Class343(-1, itemStackIn));
        }
    }

    public void onStoppedUsingItem(EntityPlayer playerIn) {
        this.syncCurrentPlayItem();
        this.netClientHandler.addToSendQueue(new Class365(Class365.Action.RELEASE_USE_ITEM, BlockPos.ORIGIN, EnumFacing.DOWN));
