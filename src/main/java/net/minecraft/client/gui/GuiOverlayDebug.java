    ...
    protected List<String> getDebugInfoRight() {
        ...
        if (this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK && this.mc.objectMouseOver.getBlockPos() != null) {
            ...
            for (Map.Entry entry : iblockstate.getProperties().entrySet()) {
                Object s1 = ((Comparable)entry.getValue()).toString();
                if (entry.getValue() == Boolean.TRUE) {
                    s1 = String.valueOf((Object)EnumChatFormatting.GREEN) + (String)s1;
                } else if (entry.getValue() == Boolean.FALSE) {
                    s1 = String.valueOf((Object)EnumChatFormatting.RED) + (String)s1;
                }
                list.add(((IProperty)entry.getKey()).getName() + ": " + (String)s1);
            }
        }
        return list;
    }
