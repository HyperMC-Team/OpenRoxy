...
implements GuiListExtended.IGuiListEntry {
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected) {
            ...
            field_148302_b.submit(new Runnable(){
                ...
                public void run() {
                    try {
                        ...
                        ServerListEntryNormal.this.server.serverMOTD = String.valueOf((Object)EnumChatFormatting.DARK_RED) + "Can't resolve hostname";
                    }
                    catch (Exception var3) {
                        ServerListEntryNormal.this.server.pingToServer = -1L;
                        ServerListEntryNormal.this.server.serverMOTD = String.valueOf((Object)EnumChatFormatting.DARK_RED) + "Can't connect to server.";
                    }
                }
            });
        }
        ...
        for (int i = 0; i < Math.min(list.size(), 2); ++i) {
            this.mc.fontRendererObj.drawString(list.get(i), x + 32 + 3, y + 12 + this.mc.fontRendererObj.FONT_HEIGHT * i, 0x808080);
        }
        String s2 = flag2 ? String.valueOf((Object)EnumChatFormatting.DARK_RED) + this.server.gameVersion : this.server.populationInfo;
        ...
        if (flag2) {
            l = 5;
            s1 = flag ? "Class1 out of date!" : "Server out of date!";
        ...
        if (i1 >= listWidth - 15 && i1 <= listWidth - 5 && j1 >= 0 && j1 <= 8) {
            this.owner.setHoveringText((String)s1);
