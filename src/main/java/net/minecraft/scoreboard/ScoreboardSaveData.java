...
extends WorldSavedData {
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        if (this.theScoreboard == null) {
            ...
            if (nbt.hasKey("Class290", 9)) {
                this.readTeams(nbt.getTagList("Class290", 10));
            }
        }
    }
    ...
    protected void readDisplayConfig(NBTTagCompound p_96504_1_) {
        for (int i = 0; i < 19; ++i) {
            if (!p_96504_1_.hasKey("slot_" + i, 8)) continue;
    ...
    protected void readObjectives(NBTTagList nbt) {
        for (int i = 0; i < nbt.tagCount(); ++i) {
            ...
            if (iscoreobjectivecriteria == null) continue;
            String s = nbttagcompound.getString("Name");
            if (s.length() > 16) {
                s = s.substring(0, 16);
            }
    ...
    protected void readScores(NBTTagList nbt) {
        for (int i = 0; i < nbt.tagCount(); ++i) {
            ...
            if (!nbttagcompound.hasKey("Locked")) continue;
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        if (this.theScoreboard == null) {
            ...
            nbt.setTag("Class290", this.func_96496_a());
